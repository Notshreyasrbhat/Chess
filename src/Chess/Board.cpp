...
// Method to find the position of the king for the given color
Square Board::findKing(Colour colour) const {
    for (Square sq = 0; sq < 64; ++sq) {
        if (GetPieceType(m_Board[sq]) == King && GetColour(m_Board[sq]) == colour) {
            return sq;
        }
    }
    return INVALID_SQUARE; // If the king is not found (should not happen)
}
