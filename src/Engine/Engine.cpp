...
bool Engine::Init() {
    ...
    std::ofstream logFile("engine_init.log", std::ios::app);
    logFile << "\n=== Engine Init Started ===" << std::endl;
    logFile << "Current time: " << std::time(nullptr) << std::endl;
    
    logFile << "Sending 'uci' command to engine..." << std::endl;
    logFile.flush();
    Send("uci\n");

    //loop till "uciok" is received with a timeout of 15 seconds
    std::string data;
    ...
    while (m_State != State::Ready) {
        ...
        if (now - startTime > timeout) {
            logFile << "TIMEOUT! Engine initialization failed - received " << commandCount << " commands but no 'uciok'" << std::endl;
            logFile << "Current state: " << (int)m_State << std::endl;
            logFile.flush();
            logFile.close();
        ...
        if (Receive(data)) {
            commandCount++;
            logFile << "Received: " << data.substr(0, 150) << (data.length() > 150 ? "..." : "") << std::endl;
            logFile.flush();
            try {
                HandleCommand(data);
            } catch (const std::exception& e) {
                logFile << "Error handling command: " << e.what() << std::endl;
                logFile.flush();
            }
        }
    ...
    logFile << "SUCCESS! Engine ready! Received " << commandCount << " initialization commands" << std::endl;
    logFile.close();
    return true;
}
